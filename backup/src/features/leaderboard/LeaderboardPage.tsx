...
import { faFire, faMusic, faPlay, faClock, faSearch, faXmark, faPlayCircle } from '@fortawesome/free-solid-svg-icons';
...
const LeaderboardPage: FC = () => {
  ...
  const charts = useMemo(() => {
    if (!searchQuery.trim()) return allCharts.slice(0, 20);
    const q = searchQuery.trim().toLowerCase();
    return allCharts.filter(c => c.name.toLowerCase().includes(q));
  ...
  useEffect(() => {
    ...
    fetchAllTracks().catch(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [selected]);
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="shrink-0 px-6 pt-6 pb-4 border-b border-white/5">
        <div className="flex items-center gap-4 mb-4">
          <h2 className="text-lg font-extrabold text-white tracking-tight shrink-0">排行榜</h2>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {charts.length === 0 && (
            <p className="text-[11px] text-white/30 py-2">没有匹配的榜单</p>
          )}
        </div>
      </div>
            ...
            {showCount < tracks.length && (
              ...
              >
                {tracks.length - showCount > 50
                  ? `点击显示 50 首`
                  : `显示剩余 ${tracks.length - showCount} 首`}
              </button>
            )}
          </>
        )}
