...
const NowPlaying: FC<NowPlayingProps> = ({ onOpenFullscreen }) => {
  ...
  return (
    <div ref={cardRef} className="clear-glass p-4 flex-[3] min-h-0 flex flex-col overflow-hidden cursor-pointer" onClick={handleExpand}>
      <div className="flex justify-between items-center mb-4 px-2 shrink-0">
        <span className="text-[11px] font-bold text-white/40 uppercase tracking-widest">正在播放</span>
        ...
        {currentSong && (
          <div className="text-center mb-6">
            ...
            <div className="flex items-center justify-center gap-3">
              <p className="text-sm text-white/50 font-medium italic">{currentSong.ar?.[0]?.name ?? '未知艺术家'}</p>
