  ...
  return (
    <div className="mt-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-sm font-bold text-white">{t('comment.title', { count: comments.length })}</span>
      </div>

      {loading ? (
        <div className="text-center py-8 text-white/30 text-sm">{t('loading')}</div>
      ) : comments.length === 0 ? (
        <div className="text-center py-8 text-white/30 text-sm">{t('comment.empty')}</div>
      ) : (
