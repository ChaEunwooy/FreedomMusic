...
const PlaylistPanel: FC = () => {
  ...
  return (
    <div className="clear-glass p-4 flex-[2] min-h-0 flex flex-col overflow-hidden "style={{marginTop: '10px',marginBottom: '20px'}} >
      <div className="flex items-center justify-between mb-4 px-2">
        <span className="text-[11px] font-bold text-white/40 uppercase tracking-widest">{t('player.up_next')}</span>
        <span className="text-[10px] text-white/30">{t('songs_count', { count: totalCount })}</span>
      </div>
      <div className="flex-1 overflow-y-auto no-scrollbar space-y-2">
        {totalCount === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-white/20">
            <p className="text-xs">{t('player.no_queue')}</p>
          </div>
        ) : (
          <>
            {currentSong && (
              <div className="flex items-center gap-2 px-2 pt-2 pb-1">
                ...
                <span className="text-[10px] font-bold text-white/50">{t('player.now_playing')}</span>
              </div>
            )}
            {currentSong && (
            ...
            {upcoming.length > 0 && (
              <>
                <div className="flex items-center gap-2 px-2 pt-2 pb-1">
                  ...
                  <span className="text-[10px] font-bold text-white/50">{t('player.upcoming')}</span>
                </div>
                {upcoming.map((song) => (
                  <PlaylistItem
                    key={song.id}
                    song={{
                      id: song.id,
                      name: song.name,
                      artist: song.ar[0]?.name ?? t('unknown'),
