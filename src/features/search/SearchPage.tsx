...
const SearchPage: FC = () => {
  const { t } = useI18n();
  const TABS = [
    { key: 1014, label: t('search.tab.all') },
    { key: 1, label: t('search.tab.songs') },
    { key: 1000, label: t('search.tab.playlists') },
    { key: 10, label: t('search.tab.albums') },
    { key: 100, label: t('search.tab.artists') },
  ] as const;
  ...
  useEffect(() => {
    ...
    setLoading(true);
    const cacheKey = `search_${tab}_${query}`;
    ...
    const fetchFromApi = async () => {
      if (tab === 1014) {
        ...
        const pl = playlistsRes.data.result?.playlists || playlistsRes.data.playlists || [];
        const al = albumsRes.data.result?.albums || albumsRes.data.albums || [];
        const ar = artistsRes.data.result?.artists || artistsRes.data.artists || [];
        setPlaylists(pl);
        setAlbums(al);
        setArtists(ar);
        let songList: any[] = [];
    ...
    const loadSearch = async () => {
      const cached = await getDailyCache<{ songs: any[]; playlists: any[]; albums: any[]; artists: any[] }>(cacheKey);
      if (reqId !== abortRef.current) return;
      if (cached) {
        ...
        <div>
          ...
          {tab === 1014 && (
            <div className="space-y-10">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {artists.length > 0 && (
                    <div className="flex items-center gap-4 bg-white/5 rounded-2xl p-5 cursor-pointer hover:bg-white/8 transition-colors">
                      ...
                      <div>
                        <p className="text-xs text-white/40 mb-0.5">{t('search.tab.artists')}</p>
                        <p className="text-lg font-bold text-white">{artists[0].name}</p>
                        <div className="flex gap-4 mt-1 text-xs text-white/35">
                          <span>{t('artist.singles', {count: artists[0].musicSize || artists[0].albumSize || '—'})}</span>
                          <span>{t('artist.followers', {count: artists[0].followeds ? formatCount(artists[0].followeds) : '—'})}</span>
                        </div>
                      </div>
                    </div>
                  )}
                  {playlists.length > 0 && (
                    <div className="flex items-center gap-4 bg-white/5 rounded-2xl p-5 cursor-pointer hover:bg-white/8 transition-colors" onClick={() => navigate(`/playlist/${playlists[0].id}`)}>
                      ...
                      <div className="min-w-0">
                        <p className="text-xs text-white/40 mb-0.5">{t('search.tab.playlists')}</p>
              ...
              {songs.length > 0 && (
                <div>
                  <SectionTitle title={t('search.tab.songs')} />
              ...
              {playlists.length > 0 && (
                <div>
                  <SectionTitle title={t('search.tab.playlists')} />
              ...
              {artists.length > 1 && (
                <div>
                  <SectionTitle title={t('search.tab.artists')} />
                  <div className="flex gap-6 overflow-x-auto no-scrollbar pb-2">
                    {artists.slice(1, 8).map((a) => (
                      <div key={a.id} className="shrink-0 flex flex-col items-center text-center cursor-pointer group w-24">
                        ...
                        <p className="text-[10px] text-white/30">{t('artist.singles', {count: a.musicSize || '—'})}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {albums.length > 0 && (
                <div>
                  <SectionTitle title={t('search.tab.albums')} />
                  <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
          ...
          {tab === 1 && (
            <div>
              <div className="grid grid-cols-[40px_1fr_80px] gap-4 px-3 py-2 text-xs text-white/25 border-b border-white/5 mb-1">
                <span className="text-center">#</span><span>{t('library.tab.songs')}</span><span className="text-right"><FontAwesomeIcon icon={faClock} /></span>
              </div>
              {songs.map((song, i) => (
                <SongRow key={song.id} song={song} index={i + 1} hoveredId={hoveredId} setHoveredId={setHoveredId}
                  isPlaying={currentSong?.id === song.id} onPlay={() => playSong(song)} />
...
function Empty() {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-white/20">
      ...
      <p className="text-sm">{t('search.no_result')}</p>
    </div>
  );
}
...
function SongRow({ song, index, hoveredId, setHoveredId, isPlaying, onPlay }: {
  ...
  return (
    <div
      ...
      className={`grid grid-cols-[40px_1fr_80px] gap-4 items-center px-3 py-1.5 rounded-lg cursor-pointer transition-colors ${
    ...
    >
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          <p className={`text-sm truncate ${isPlaying ? 'text-cyan-400' : 'text-white/80'}`}>
            {song.name}
            <span className="text-white/30 font-normal ml-2">- {artists.map((a: any) => a.name).join(' / ')}</span>
          </p>
        </div>
      </div>
      <div className="flex items-center justify-end gap-2" onClick={(e) => e.stopPropagation()}>
        <p className="text-xs text-white/25 tabular-nums">{formatDuration(song.dt || song.duration || 0)}</p>
  ...
  return (
    <div onClick={onClick} className="flex items-center gap-3 bg-white/5 rounded-xl p-3 hover:bg-white/8 transition-colors cursor-pointer">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs text-white/35 mt-0.5 truncate">{t('songs_count', {count: item.trackCount || 0})} · {item.playCount ? t('play_count', {count: formatCount(item.playCount)}) : ''}</p>
      </div>
    </div>
  );
}
